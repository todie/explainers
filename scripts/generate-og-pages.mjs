...
// Emit dist/_redirects so Cloudflare Pages serves the prerendered per-route
// HTML for /{slug} (no trailing slash) instead of falling through to the SPA
// shell. Without this, link unfurlers (Slack, Twitter, Discord) hit /harness,
// the SPA wildcard catches it before /harness/index.html, and they receive
// the homepage OG tags. Each explicit rewrite must precede the SPA fallback.
const redirectLines = [
  '# Auto-generated by scripts/generate-og-pages.mjs — do not hand-edit.',
  '# Per-route rewrites must precede the SPA fallback so link unfurlers',
  '# receive the prerendered HTML with route-specific OG tags.',
  '',
  ...EXPLAINERS.map((ex) => `${ex.path}  ${ex.path}/index.html  200`),
  ...extraPages.map((p) => `/${p.id}  /${p.id}/index.html  200`),
  '',
  '# SPA fallback for everything else (deep links into client-side routes).',
  '/*  /index.html  200',
  '',
].join('\n')

writeFileSync(join(DIST, '_redirects'), redirectLines)
console.log(`Wrote dist/_redirects (${total} explicit rewrites + SPA fallback).`)
