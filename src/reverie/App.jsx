  ...
  return (
    <div style={{
      ...
      <header style={{
        ...
        }} className="reverie-particles">
          {Array.from({ length: 20 }, (_, i) => (
            <div key={i} className="reverie-particle" style={{
              position: 'absolute',
              left: `${5 + (i * 47) % 90}%`,
              top: `${10 + (i * 31) % 80}%`,
              width: 2 + (i % 3),
      ...
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .reverie-particle { animation: none !important; }
          .reverie-particles + div { animation: none !important; }
        }
      `}</style>
    </div>
  )
